...
export function ProductDescription({ product }: { product: Product }) {
  ...
  let amount = 0,
    lowestAmount = 0,
    highestAmount = 0
  const currencyCode = currency?.code?.toUpperCase() || 'EUR'
  const priceField = `priceIn${currencyCode}` as keyof Product
  const hasVariants = product.enableVariants && Boolean(product.variants?.docs?.length)

  if (hasVariants) {
    const priceField = `priceIn${currencyCode}` as keyof Variant
    ...
    if (variantsOrderedByPrice && variantsOrderedByPrice.length > 0) {
